> 归档时间：2026-09-18（CST）。2026-08-20 CPU 基线记录，2026-09-18 归档；不是当前 NPU PR 的验收。
>
> 原始来源：`Tracking/cpu_provenance_visualization_demo.md`。
...
python /home/z50063656/TorchNpu-Inductor-Provenance/docs/cpu/cpu_provenance_demo.py
```
...
- [Provenance 三栏高亮器](./provenance_tracking.html)
- [Kernel stack readable 页面](./kernel_stack_readable.html)
- [原始节点映射 JSON](./node_mappings.json)
- [Kernel stack JSON](./kernel_stack_traces.json)

本仓精选保留独立三栏页面，没有复制整个临时 tlparse 目录。下载本目录 HTML 后直接
打开即可；若自己重跑，仍可从生成的 `index.html` 进入 **Provenance Tracking**。
以上命令保留原机环境路径，其他机器应改用自己的匹配环境和仓库路径。三栏分别是：
